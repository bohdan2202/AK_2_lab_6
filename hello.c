...
MODULE_AUTHOR("Karmaniuk Bohdan IO-04");
MODULE_DESCRIPTION("Lab AK_2 nomber_6");
...
MODULE_PARM_DESC(num_of_greet, "Num 'Hellooo, world!'");
...
static int __init hello_init(void)
{
    if (num_of_greet == 0 || (num_of_greet >= 5 && num_of_greet <= 10)) {
        printk(KERN_WARNING "parameter is 0 or among 5 and 10");  
    }

    if (num_of_greet > 10) {
        printk(KERN_ERR "parameter is very large");
        return -EINVAL;
    }
  
    int i;
    for (i = 0; i < num_of_greet; i++) {
        printk(KERN_EMERG "hellooo, world!\n");
        new_event();
    }
  return 0;
}
